...
async def raise_pr(
    ...
    fix_branch = f"stagecraft/fix-{run.github_run_id}"
    try:
        ...
        await github.commit_fix(
            ...
            message=f"fix: Stagecraft AI-suggested fix for workflow run #{run.github_run_id}",
            current_sha=current_sha,
        )

        pr_body = (
            f"## Root Cause\n{remediation.root_cause}\n\n"
            "## Changes\nThis fix was suggested by Stagecraft (AWS Bedrock - Amazon Nova)."

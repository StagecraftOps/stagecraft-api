...
async def raise_pr(
    ...
    After the worker stores a suggested fix (status='analyzed'), the user
    reviews it here and clicks "Raise PR" to actually create the GitHub branch,
    commit, and pull request. This keeps write-scope GitHub operations
    user-authorized, not automated.
    """
    result = await db.execute(select(Remediation).where(Remediation.id == remediation_id))
    remediation = result.scalar_one_or_none()
    if not remediation:
    ...
    if not remediation.suggested_yaml:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="No suggested fix available yet — analysis may still be running",
        )
    ...
    try:
        ...
        pr_body = (
            ...
            "## Changes\nThis fix was suggested by aGorA (AWS Bedrock — Claude 3.5 Sonnet)."
